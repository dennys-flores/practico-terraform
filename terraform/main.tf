...
locals {
  ...
  #ruta_private_key = "~/Descargas/Ansible.pem"
  nombre_key = "Ansible"
  usuario_ssh = "ubuntu"
}
...
resource "aws_instance" "mi_app_nestjs" {
  ...
  provisioner "local-exec" {
    command = "ansible-playbook -i ${self.public_ip}, --private-key ${var.ruta_private_key} ../ansible/main.yml"
  }
}
